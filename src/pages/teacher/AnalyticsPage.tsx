...
import {
  useTeacherAnalyticsOverview,
  useTeacherAnalyticsClasses,
  useTeacherAnalyticsClass,
  useTeacherAnalyticsStudents,
  useTeacherAnalyticsStudent,
  useTeacherStudentAssessments,
  useTeacherStudentActivities,
  useTeacherStudentStreak,
} from "@/hooks/useTeacherAnalytics";
import type {
  TeacherOverview,
  TeacherClass,
  TeacherClassDetailed,
  StudentItem,
...
const COLORS = {
  ...
  cardGradients: {
    violet:
      "from-violet-500/10 via-purple-500/10 to-indigo-500/10 hover:from-violet-500/20 hover:via-purple-500/20 hover:to-indigo-500/20",
...
export default function TeacherAnalyticsPage() {
  ...
  const { data: overviewData, isLoading: overviewLoading } =
    useTeacherAnalyticsOverview(user?.id, days);

  const { data: classesData, isLoading: classesLoading } =
    useTeacherAnalyticsClasses(user?.id, {
      days,
      search: classSearchQuery || undefined,
    });

  const { data: classDetailData, isLoading: classDetailLoading } =
    useTeacherAnalyticsClass(selectedClassId || undefined, days);

  const { data: studentsData, isLoading: studentsLoading } =
    useTeacherAnalyticsStudents(user?.id, {
      days,
      limit: 20,
      page: studentPage,
      ...
      risk_level:
        riskFilter !== "all"
          ? (riskFilter as "low" | "medium" | "high")
          : undefined,
    });

  const { data: studentDetailData, isLoading: studentDetailLoading } =
    useTeacherAnalyticsStudent(selectedStudentId || undefined, days);

  // Search handlers
  const handleClassSearch = () => setClassSearchQuery(classSearchInput);
    ...
    <div className="space-y-6 animate-in fade-in duration-500 relative pb-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Insights for your assigned classes and students
              </p>
            </div>
          </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3 h-14 p-1 bg-muted/50 backdrop-blur-sm rounded-2xl mb-8">
          <TabsTrigger
            value="overall"
            className="rounded-xl gap-2 text-base data-[state=active]:bg-gradient-to-r data-[state=active]:from-violet-500 data-[state=active]:to-purple-500 data-[state=active]:text-white"
          >
          ...
          <TabsTrigger
            value="class"
            className="rounded-xl gap-2 text-base data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-cyan-500 data-[state=active]:text-white"
          >
          ...
          <TabsTrigger
            value="student"
            className="rounded-xl gap-2 text-base data-[state=active]:bg-gradient-to-r data-[state=active]:from-emerald-500 data-[state=active]:to-teal-500 data-[state=active]:text-white"
          >
...
function OverallView({
  ...
  const StatCard = ({ title, value, subtext, icon: Icon, gradient }: {
    title: string;
    value: string | number;
    subtext?: string;
    ...
    <Card
      className={`relative overflow-hidden border-0 shadow-lg bg-gradient-to-br ${gradient} group hover:scale-[1.02] transition-transform duration-300`}
    >
      <CardContent className="pt-6 relative z-10">
        ...
        <p className="text-3xl font-bold text-foreground dark:text-white mb-1 tracking-tight">
          {value}
        </p>
        ...
        {subtext && (
          <p className="text-xs text-muted-foreground/80 dark:text-white/50 mt-2">
            {subtext}
          </p>
        )}
      </CardContent>
...
function ClassWiseView({
  ...
  classDetail?: TeacherClassDetailed;
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    ...
                    >
                      {classDetail.total_students} students
                    </Badge>
                    <Badge variant="destructive" className="shadow-sm">
                      {classDetail.at_risk_count} at risk
                    </Badge>
                  </div>
                </div>
              </div>
              <div className="text-right hidden md:block">
                ...
                <div className="flex items-baseline justify-end gap-1">
                  <p className="text-4xl font-bold text-primary">
                    {classDetail.metrics.avg_wellbeing}
                  </p>
              ...
              >
                <BarChart
                  data={[
                    {
                      name: "Wellbeing",
                      class: classDetail.metrics.avg_wellbeing,
                      school: 78,
                    },
                    {
                      name: "Assessment",
                      class: classDetail.metrics.assessment_completion,
                      school: 65,
                    },
                    {
                      name: "Activity",
                      class: classDetail.metrics.activity_completion,
                      school: 72,
                    },
                  ]}
                >
            ...
            <Card className="h-full">
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <Progress value={classDetail.metrics.avg_wellbeing ?? 0} className="h-2" />
                </div>
                <div>
                  <div className="flex justify-between mb-2">
                    <span className="text-sm text-muted-foreground">Assessment Rate</span>
                    <span className="font-bold text-cyan-600">
                      {classDetail.metrics.assessment_completion}%
                    </span>
                  </div>
                  <Progress
                    value={classDetail.metrics.assessment_completion}
                ...
                <div>
                  <div className="flex justify-between mb-2">
                    ...
                    <span className="font-bold text-emerald-600">
                      {classDetail.metrics.activity_completion}%
                    </span>
                  </div>
                  <Progress
                    value={classDetail.metrics.activity_completion}
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    {classDetail.students?.length || 0} students
                  </CardDescription>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div className="p-2 rounded-lg bg-muted/50 text-center">
                  <p className="text-xl font-bold text-primary">
                    {classData.metrics.avg_wellbeing}%
                  </p>
                  <p className="text-xs text-muted-foreground">Wellbeing</p>
                </div>
                <div className="p-2 rounded-lg bg-muted/50 text-center">
                  <p className="text-xl font-bold text-cyan-600">
                    {classData.metrics.assessment_completion}%
                  </p>
...
function StudentDetailView({
  ...
  // Fetch additional data using teacher hooks
  const { data: assessmentsData } = useTeacherStudentAssessments(studentId, true);
  const { data: activitiesData } = useTeacherStudentActivities(studentId);
  const { data: streakData } = useTeacherStudentStreak(studentId);
  ...
  // Type the student detail
  const student = studentDetail as {
    name: string;
    email?: string;
    class?: { name: string };
    current_metrics: {
    ...
    wellbeing_trend?: { week: string; score: number }[];
  } | null;
  ...
  const { current_metrics, engagement, wellbeing_trend } = student;
  ...
  const completionData = [
    {
      name: "Assessments",
      completed: engagement.assessments_completed,
      total: engagement.assessments_total,
      color: "#06b6d4",
    },
    {
      name: "Activities",
      completed: engagement.activities_completed,
      total: engagement.activities_total,
      color: "#10b981",
    },
  ].filter((item) => item.total > 0);

  // Type the streak data
  const streak = streakData as {
    current_streak: number;
    max_streak: number;
    total_active_days: number;
    daily_history: { date: string; app_opened: boolean; activity_completed: boolean }[];
  } | null;

  // Type the assessments data
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <div className="flex items-center gap-1 text-orange-600">
                    ...
                    <span className="font-bold">
                      {current_metrics.daily_streak} day streak
                    </span>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center p-3 rounded-xl bg-background/50">
                <p className="text-3xl font-bold text-primary">
                  {current_metrics.wellbeing_score ?? 0}%
                </p>
                <p className="text-xs text-muted-foreground">Wellbeing</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-background/50">
                <p className="text-3xl font-bold text-orange-600">
                  {current_metrics.daily_streak}
                </p>
                <p className="text-xs text-muted-foreground">Current Streak</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-background/50">
                <p className="text-3xl font-bold text-blue-600">
                  {engagement.total_app_openings}
                </p>
                <p className="text-xs text-muted-foreground">App Opens</p>
              </div>
              <div className="text-center p-3 rounded-xl bg-background/50">
                <p className="text-3xl font-bold text-emerald-600">
                  {engagement.avg_session_time || 0}m
                </p>
        ...
        <Card className="overflow-hidden">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2 p-4 bg-muted/20 rounded-xl justify-center">
              {streak.daily_history.slice(0, 30).map((day, index) => {
                const intensity = day.activity_completed
                ...
                return (
                  <Tooltip key={index}>
                    <TooltipTrigger asChild>
                      <div
                        className={`w-8 h-8 rounded-md transition-colors cursor-pointer ${intensity}`}
                      />
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="font-bold">{day.date}</p>
                      <p>
                        {day.activity_completed
                          ? "Completed Activity"
                          : day.app_opened
                          ? "Visited App"
                          : "Inactive"}
            ...
            <div className="flex justify-between items-center mt-4 px-2">
              <div className="flex gap-4">
                ...
                <div className="text-center">
                  <p className="text-2xl font-bold text-blue-600">{streak.max_streak}</p>
              ...
              >
                <LineChart data={wellbeing_trend}>
                  ...
                  <Line
                    ...
                    strokeWidth={3}
                    dot={{ fill: "var(--color-score)", strokeWidth: 2, r: 5 }}
                  />
                </LineChart>
              </ChartContainer>
            </CardContent>
        ...
        <CardContent>
          <Tabs defaultValue="assessments" className="w-full">
              ...
              {assessments?.assessments?.length ? (
                assessments.assessments.map((assessment, index) => (
                  <div
                    ...
                    className="p-4 rounded-xl border-2 hover:border-primary/30 hover:shadow-md transition-all bg-card"
                  >
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="w-16">
                          <Progress
                            value={(assessment.total_score / assessment.max_score) * 100}
                            className="h-2"
                          />
                        </div>
                      </div>
                      ...
                      <div className="mt-4 ml-13 p-4 bg-muted/30 rounded-xl border">
                        ...
                        <div className="space-y-2">
                          {assessment.responses.map((r, i) => {
                              ...
                              if (typeof val === "string") {
                                try {
                                  ...
                                  if (parsed && typeof parsed === "object") {
                                    return (
                                      parsed.text || parsed.value?.toString() || val
                                    );
                                  }
                                } catch {
                                  return val;
                            ...
                            const displayValue =
                              getDisplayValue(r.answer_text) !== "N/A"
            ...
            <TabsContent value="activities" className="space-y-3">
                    ...
                    <>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <DialogDescription className="flex items-center gap-2 mt-1">
                              ...
                              <Badge
                                className={
                                  selectedActivity.status === "VERIFIED"
                                    ? "bg-green-100 text-green-700"
                      ...
                      <div className="space-y-4 mt-4">
                        ...
                        {(selectedActivity.student_response?.text ||
                          selectedActivity.student_response?.media_url ||
                        ...
                        {selectedActivity.feedback && (
                          <div className="p-4 bg-green-50/50 dark:bg-green-950/20 rounded-xl border border-green-200 dark:border-green-800">
                            ...
                            {selectedActivity.verified_by && (
                              <p className="text-xs text-muted-foreground mt-2">
                                Verified by: {selectedActivity.verified_by.name}
                              </p>
                            )}
                          </div>
                        )}
              ...
              <Dialog
                open={!!attachmentUrl}
                onOpenChange={(open) => !open && setAttachmentUrl(null)}
              >
